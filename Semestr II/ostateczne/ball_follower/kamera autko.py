...
for cameraFrame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if not paused:
        ...
        #dodanie blura poprawia działanie wykrywania jeśli to zabrać powinno być z lekka gorzej
        ...
        if circles is not None:
            ...
            if contours is not None:
                for cnt in contours:
                    ...
                    x1 = w/2
                    y1 = h/2
                    #jeśli warunek spełniony to porównujemy środki z metody
                    if  len(approx) > 7:
                        foundedCenter = (x, y)
                        for i in circles[0, :]:
                            cv2.circle(resizedColor, (i[0],i[1]), i[2], (255, 0, 255), 3)
                            #porównujemy środki jeśli są takie same lub podobne między sobą 
                            #jeśli spełnia warunek to wprowadzamy do listy pasujących conturow kulek
                            distance = math.sqrt( (x1 -  y1) * (x1 - y1) + (i[0] - i[1])*(i[0] - i[1]) )
                            if(distance < 3):
                                newcountours.append(cnt)

            #dalej program dziala tak jak wcześniej ale dla nowych countorów
            #teraz patrzymy który z tych przefiltrowanych contorów jest nabliżej            
            boundingBoxes = []
            biggestObject_BoundingBox = None
            biggestObjectMiddle = None
            if newcountours: #mozliwe iz te 2 obiekty trzeba wstawić do pętli
